...
const Privacy = () => {
  const parsed = useMemo(() => {
    ...
    try {
      ...
      const content = privacyRaw.replace(/^---[\s\S]*?---\s*/, "").trim();
      return { data: { title: data.title ?? "Política de Privacidade", lastUpdated: data.lastUpdated ?? "", version: data.version ?? "" }, content };
    } catch (err) {
